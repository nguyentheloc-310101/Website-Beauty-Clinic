...
import { clearOrder, getOrderStorage } from "@/services/stream/order-service";
...
const BookingPage = () => {
    ...
    const [service, setService] = useState<string[]>(
        getOrderStorage()
            ? getOrderStorage()?.map((service) => service.service.id)
            : []
    );
    ...
    const onOk = async () => {
        ...
        if (
            ...
            message.warning(
                "Ngày bạn chọn không hợp lệ, vui lòng chọn ngày ở hiện tại hoặc trong tương lai."
            );
            ...
            service &&
                service.map(async (item) => {
                    ...
                    const { data: dataBooking, error: errorBooking } =
                        await supabase.from("bookings").insert(new_booking);
                    if (errorBooking) {
    ...
    const fetchClinics = async () => {
        const { data }: any = await supabase.from("clinics").select("*");
