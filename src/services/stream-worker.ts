...
export async function getStreamKey(user_id:any,username:string, avatar: string){
     ...
     console.log(res)
     if (res.status === 200){
          let stream_info = await supabase.from('stream_info')
          .insert({
               user_id,
          ...
          .select();
          if (stream_info?.data){
               let token = await axios.get(backend_url + `/stream/getToken?activityId=${stream_info?.data[0].activity_id}`)
               console.log(token)
               if (token){
                    await supabase.from("stream_info").update({token: token.data.data.Result.Token}).eq('user_id',user_id)
               }
